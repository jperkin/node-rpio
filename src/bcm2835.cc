...
int bcm2835_init(void)
{
    ...
    // GPIO:
    gpio_map = (char *)mapmem("gpio", BCM2835_BLOCK_SIZE, memfd, BCM2835_GPIO_BASE);
    if (gpio_map == MAP_FAILED) goto exit;
    gpio = (volatile unsigned *)gpio_map;

    // PWM
    pwm_map = (char *)mapmem("pwm", BCM2835_BLOCK_SIZE, memfd, BCM2835_GPIO_PWM);
    if (pwm_map == MAP_FAILED) goto exit;
    pwm = (volatile unsigned *)pwm_map;

    // Clock control (needed for PWM)
    clk_map = (char *)mapmem("clk", BCM2835_BLOCK_SIZE, memfd, BCM2835_CLOCK_BASE);
    if (clk_map == MAP_FAILED) goto exit;
    clk = (volatile unsigned *)clk_map;
    
    pads_map = (char *)mapmem("pads", BCM2835_BLOCK_SIZE, memfd, BCM2835_GPIO_PADS);
    if (pads_map == MAP_FAILED) goto exit;
    pads = (volatile unsigned *)pads_map;
    
    spi0_map = (char *)mapmem("spi0", BCM2835_BLOCK_SIZE, memfd, BCM2835_SPI0_BASE);
    if (spi0_map == MAP_FAILED) goto exit;
